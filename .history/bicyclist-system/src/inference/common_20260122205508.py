...
class FrameLabelThresholds:
    ...
    IMPORTANT (Route A design):
      ...
      - Therefore, crosswalk uses a POINT-IN-ROI rule (bottom-edge 3 points) by default.
    """
    T_ROAD: float = 0.30
    T_BIKE: float = 0.25
    T_SIDE: float = 0.30
...
def frame_space_label(
    ...
    Crosswalk rule (robust):
      - Use bottom-edge 3-point-in-ROI ONLY (no overlap threshold),
    ...
    # --- 0) Ignore-zone: if exists and bc in ignore => unknown (drop later by gate) ---
    if has_roi("ignore_zone") and roi.point_in_roi(bc, "ignore_zone"):
        return "unknown"

    # --- 1) Crosswalk: bottom-edge 3-point rule ---
